...
COLUNAS = [
    ...
    ("Valor Comercial", 14), ("Alíq. Interestadual", 12), ("Categoria", 30),
    ("MVA Original", 12), ("MVA Ajustada", 12), ("Base de Cálculo", 14),
    ("Alíq. Interna BA", 12), ("Crédito de Origem", 14), ("Antecipação Devida", 16),
    ("Observações", 44),
]

COLS_MOEDA = {14, 19, 21, 22}
COLS_PERCENTUAL = {15, 17, 18, 20}
...
def _observacoes(item: ItemAntecipacao, res: ResultadoAntecipacaoItem) -> str:
    ...
    if not item.icms_destacado:
        if item.percentual_credito_simples is not None:
            obs.append(
                f"Simples Nacional (CSOSN com permissão de crédito) — {item.percentual_credito_simples:.2f}% "
                f"de ICMS embutido no preço (art. 23 da LC 123/2006) usado como crédito de origem."
            )
        else:
            obs.append(
                "Simples Nacional sem percentual de crédito de ICMS informado na nota — crédito de "
                "origem considerado 0% (posição conservadora)."
            )
...
def gerar_excel(
    ...
    for item, res in linhas:
        valores = [
            ...
            round(item.valor_comercial, 2), res.aliquota_interestadual, categoria_label,
            res.mva_original, res.mva_ajustada, round(res.base_calculo, 2),
